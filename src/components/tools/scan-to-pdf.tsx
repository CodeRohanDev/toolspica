  ...
  async function generate() {
    ...
    try {
      ...
      for (const { file } of items) {
        // Normalize every scan to JPEG via canvas so any input format
        // (HEIC on iPhone, PNG screenshot, etc.) embeds reliably.
      ...
      downloadPdfBytes(pdfBytes, "scanned-document.pdf");
    } finally {
      setProcessing(false);
    }
  }
  ...
  return (
    <div className="rounded-xl border bg-card p-5 sm:p-6">
      ...
      >
        ...
        <p className="text-sm font-medium">Take a photo or choose images to scan (multiple pages)</p>
