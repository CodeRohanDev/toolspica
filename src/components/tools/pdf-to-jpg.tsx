...
export function PdfToJpg() {
  ...
  async function convert() {
    ...
    try {
      ...
      if (entries.length === 1) {
        downloadBytesFile(entries[0].data, `${stripPdfExtension(file.name)}.jpg`, "image/jpeg");
      } else {
  ...
  return (
    <div className="rounded-xl border bg-card p-5 sm:p-6">
      ...
      <Button type="button" className="mt-4" onClick={convert} disabled={!file || processing}>
        ...
        {processing ? "Converting..." : pageCount > 1 ? `Convert ${pageCount} pages to JPG (ZIP)` : "Convert to JPG"}
      </Button>
    </div>
  );
}
