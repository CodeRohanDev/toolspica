...
export function HtmlMinifier() {
  const [input, setInput] = React.useState("<div>\n  <p>Hello   world</p>\n  <!-- comment -->\n</div>");
  const output = React.useMemo(() => minifyHtml(input), [input]);
  const saved = input.length ? Math.round((1 - output.length / input.length) * 100) : 0;

  return (
    <div className="rounded-xl border bg-card p-5 sm:p-6">
      <Textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Paste HTML..." className="min-h-[180px] font-mono text-sm" />
