...
export function PdfToCsv() {
  const convert = React.useCallback(async (file: File) => {
    ...
    return { blob, name: `${stripPdfExtension(file.name)}.csv` };
  }, []);
  ...
  return (
    <div className="rounded-xl border bg-card p-5 sm:p-6">
      <BatchUploadZone accept="application/pdf,.pdf" onFilesSelect={addFiles} label="Drop PDFs to convert to CSV" />

      <BatchFileList items={items} onRemove={removeItem} zipName="converted-csv.zip" />
