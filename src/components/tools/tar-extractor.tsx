...
export function TarExtractor() {
  ...
  async function handleFile(picked: File) {
    ...
    try {
      ...
      const result = await extractArchive(buffer, "in.tar");
      if (result.length === 0) throw new Error("No files found — this may not be a valid TAR archive.");
      setEntries(result);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Couldn't extract this TAR file.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="rounded-xl border bg-card p-5 sm:p-6">
      <MediaUploadZone file={file} onFileSelect={handleFile} onClear={() => setFile(null)} accept=".tar" kind="archive" />
