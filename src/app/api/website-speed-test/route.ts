...
import { measureWebsiteSpeed } from "@/lib/server/network-tools";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await measureWebsiteSpeed(targetUrl);
    return Response.json({ url: targetUrl.toString(), ...result });
  } catch (err) {
    const status = err instanceof UnsafeTargetError ? 400 : 502;
    return Response.json(
      { error: err instanceof Error ? err.message : "Could not measure speed for this URL." },
      { status }
    );
  }
}
